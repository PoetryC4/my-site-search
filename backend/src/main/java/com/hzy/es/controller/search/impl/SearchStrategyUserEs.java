...
@Component
public class SearchStrategyUserEs implements SearchStrategy {
    private static SearchStrategyUserEs searchStrategyUserEs;
    ...
    public void init() {
        searchStrategyUserEs = this;
        searchStrategyUserEs.userService= this.userService;
    ...
    public Page<UserVO> doSearch(CommonQueryRequest commonQueryRequest, HttpServletRequest request) {
        ...
        userQueryRequest.setSearchText(commonQueryRequest.getSearchText());

        Page<User> userPage = searchStrategyUserEs.userService.searchFromEs(userQueryRequest);
        Page<UserVO> userVOPage = new Page<>(commonQueryRequest.getCurrent(), commonQueryRequest.getPageSize(), userPage.getTotal());
        List<UserVO> userVO = searchStrategyUserEs.userService.getUserVO(userPage.getRecords());
